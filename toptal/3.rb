...
def solution(a)
  for index in [*3..a.length-1]
    if cross_boundaries(a, index) || cross_inner_path(a, index)
      return index + 1 # return the move number
    end
  end
...
def cross_boundaries(a, index)
  a[index-1] <= a[index-3] && a[index] >= a[index-2]
end

def cross_inner_path(a, index)
  a[index-1] <= a[index-3] && a[index-1] >= a[index-3] - (a[index-5] || 0) && a[index] >= a[index-2] - (a[index-4] || 0)
end

solution([1,3,2,5,4,4,6,3,2])
